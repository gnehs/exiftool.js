...
(function () {
  ...
  (function () {
    ...
    function getExifFromNodeBuffer(buffer, callback) {
      var binaryResponse = new BinaryFile(
        buffer.toString("binary"),
        0,
        Math.min(buffer.length, _readSize)
      );
      ...
      while (iOffset < iLength) {
        ...
        if (iMarker == 22400) {
          ...
          if (headerAsString.indexOf("http://ns.adobe.com/xap/1.0/") != -1) {
            var sXmpData = oFile.getStringAt(
              iOffset + 33,
              oFile.getShortAt(iOffset + 2, true) - 31
            );

            var xmlDoc;
            try {
              sXmpData = sXmpData.trim();
              sXmpData = sXmpData.substr(
                sXmpData.indexOf("<"),
            ...
            if (xmlDoc != null) {
              $(["SerialNumber", "ImageUniqueID"]).each(function (
                index,
                tagName
              ) {
    ...
    function calculateOffsetBase(
      ...
      var expectedFirstIFDValueLocation = iEntriesPosition + 2 + iEntries * 12;
      var nextIFDvalue = oFile.getLongAt(
        expectedFirstIFDValueLocation + iTIFFStart,
        bBigEnd
      );
      ...
      var expectedFirstIFDValueLocationHex =
        expectedFirstIFDValueLocation.toString(16);
      ...
      for (var i = 0; i < iEntries; i++) {
        ...
        if (
          ...
          var differenceBetweenExpectedAndActual =
            expectedFirstIFDValueLocation - ifdPointer;
          var differenceBetweenExpectedAndActualHex =
            differenceBetweenExpectedAndActual.toString(16);
    ...
    function readTagValue(
      ...
      var iValueOffset =
        oFile.getLongAt(iEntryOffset + 8, bBigEnd) + iTIFFStart + iOffsetBase;
      ...
      if (oTags.MakerNoteIFDPointer && EXIF.MakeInfo[oTags.Make]) {
        ...
        if (oMakeInfo.MakerNoteByteAlignHeaderOffset) {
          ...
          if (iByteAlign == 0x4949) {
            ...
            console.log(
              "Failed to find legal endianess in makernotes. Using default"
            );
        ...
        var headerString = tidyString(
          oFile.getStringAt(iTIFFOffset + oTags.MakerNoteIFDPointer, 8)
        );
        ...
        if (oMakeInfo.SerialFoundAtStartOfMakerNotes) {
          var serialNumber = tidyString(
            oFile.getStringAt(iTIFFOffset + oTags.MakerNoteIFDPointer, 16)
          );
          if (oMakeInfo.InvalidSerialStart) {
            var startOfSerial = serialNumber.substr(
              0,
        ...
        if (oMakeInfo.InternalSerialWithinIFDArray) {
          ...
          if (IFDArray) {
            internalSerialNumber =
              IFDArray[oMakeInfo.InternalSerialWithinIFDArrayElement];
            // if (serialNumber) {
    ...
    function formatSerialNumber(tags, serial) {
      ...
      switch (tags.Make) {
        ...
        case "FUJIFILM":
          ...
          var returnSerial =
            returnSerial.substr(0, iDateIndex) +
            " " +
            year +
            ":" +
            month +
        ...
        case "Panasonic":
          var year =
            String.fromCharCode(serial[3]) + String.fromCharCode(serial[4]);
          var month =
            String.fromCharCode(serial[5]) + String.fromCharCode(serial[6]);
          var date =
            String.fromCharCode(serial[7]) + String.fromCharCode(serial[8]);
